...
export default function MainComponent() {
  ...
  const filterUsers = (arrayOfUsers: FormattedRandomUser[]) => arrayOfUsers.filter((user) => {
      ...
      <div className="flex justify-center rounded-lg">
        <label htmlFor="search-bar">
          <input
            ...
            className="rounded-md mb-2 text-center"
            type="search"
            value={searchBar}
            name="searchBar"
            onChange={(e) => {
        ...
        <button
          className=" ml-1 bg-indigo-600 px-3 h-6 rounded-lg text-white font-medium shadow-md"
          ...
          <div className="flex flex-wrap w-auto justify-center">
            {users.length !== 0 && filterUsers(users).map((e) => (
              <div key={e.username} className="bg-slate-50 w-1/4 rounded-lg mb-2 shadow-sm px-3 py-2 mx-1">
                ...
                <img src={e.picture} className="rounded-full pb-1 mx-auto" alt="user" />
